...
def metrics(data):
    ...
    return {
        ...
        "annualized_excess": float((1 + r).prod() ** (252 / len(r)) - (1 + bench).prod() ** (252 / len(bench))),
...
def main():
    ...
    for code, group in frame.groupby("index_code", sort=False):
        ...
        group["cost"] = group["turnover"] * ONE_WAY_COST
        group["strategy_return"] = group["target_position"] * group["benchmark_return"] - group["cost"]
        group["index_name"] = code
        rows.append(group[["trade_date", "index_code", "index_name", "score", "target_position", "turnover", "cost", "benchmark_return", "strategy_return"]])
    ...
    summary["protocol"] = {"signal": "T close", "execution": "T+1 open", "holding_return": "T+1 open to T+2 open", "one_way_cost": ONE_WAY_COST, "rule": "equal vote of fixed 20d momentum, 60d close/MA, 120d momentum"}
